...
if($_POST['proxy']!=null){//使用了代理
  if(($_FILES['fmfile']!=null)||$_POST['fmurl']!=null){
    ...
    if($_FILES['fmfile']!=null){//上传文件
      ...
      //$filename=iconv("utf-8","gb2312",$f['name']); 
      //test
      //test
      //$dest=$dest_dir.'/'.$timestamp.$f['name'];//设置文件名为日期加上文件名避免重复
      $dest=$dest_dir.'/'.$timestamp.$filetype;//设置文件名为日期加上文件名避免重复  
      $r=move_uploaded_file($f['tmp_name'],$dest); 
      chmod($dest, 0755);//设定上传的文件的属性
      $benziurl=$dest;
      echo "<script>alert('fuck 1！');</script>";
    }
    else{//链接
   ...
   /* $i->version=1;
    ...
    $i->origin=$author;
    $info=json_encode($i);*/
    echo "<script>alert('fuck 2！');</script>";
    if((!is_numeric($price))||(!is_numeric($amount))){
      echo "<script>alert('价格和数量请输入数字！');</script>";
    }
    else if($addurl==""){
      echo "<script>alert('fuck 3！');</script>";
    }
    else{
      echo "<script>alert('fuck 4！');</script>";
      $sqlAdd="INSERT INTO benzi (name,groupname,groupurl,author,benziurl,time,orient,type, proxy,theme, price, amount, display,addurl) 
      VALUES ('".$name."','".$groupname."', '".$groupurl."','".$author."','".$benziurl."','".$timestamp."', '".$orient."', 0, 0,'".$theme."', ".$price.", ".$amount.", 0,'".$addurl."');";
      $db->query($sqlAdd);
    }
  }
  else{
  ...
  if(($_FILES['fmfile']!=null)||$_POST['fmurl']!=null){
   ...
   /* $i->version=1;
    ...
    if((!is_numeric($price))||(!is_numeric($amount))){
      ...
      VALUES ('".$name."','".$groupname."', '".$groupurl."','".$author."','".$benziurl."','".$timestamp."', '".$orient."', 0, 0,'".$theme."', ".$price.", ".$amount.", 0);";
      $db->query($sqlAdd);
    }
  }
  else{
                ...
                <form enctype="multipart/form-data" action="submitbenzi.php" method="post">
            	  <table align="center">
                    ...
                    <tr>
                     ...
                     <tr id='file'>
                      <td>封面图片上传</td>
                      <td><input type="hidden" name="max_file_size" value="400000"><input type="file" name='fmfile' id='fmfile'></td>
